...
class thread_pool_hello {
    std::atomic_bool done;
    std::mutex m;
    std::condition_variable cv;
    std::queue<Hello*> work_queue;
    ...
    public:
        int n_threads = 0;
        thread_pool_hello(int n_threads): done(false) {
            n_threads = n_threads;
            try {
                for(int i  = 0; i < n_threads; i++) {
                    threads.push_back(
                        std::thread(&thread_pool_hello::worker_thread, this)
                    );
                }
            } catch(...) {
